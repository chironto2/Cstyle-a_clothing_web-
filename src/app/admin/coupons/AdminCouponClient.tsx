...
const couponFormSchema = z.object({
  ...
  usageLimit: z.preprocess(
    ...
    z
      .number()
      .int()
      .min(0, 'Usage limit must be a non-negative integer.')
      .optional()
      .nullable()
  ),
...
export default function AdminCouponClient({
  ...
  const fetchCouponsAndRefresh = async () => {
    ...
    try {
      ...
      toast({
        title: 'Coupons Refreshed',
        description: 'The list of coupons has been updated.',
      });
    } catch (error) {
      toast({
        title: 'Error',
        description: 'Could not refresh coupons.',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
    ...
    if (coupon) {
      form.reset({
        ...
        expiryDate: coupon.expiryDate
          ? parseISO(coupon.expiryDate as string)
          : new Date(),
        minPurchaseAmount:
  ...
  const onSubmit: SubmitHandler<CouponFormData> = async (data) => {
    ...
    const apiEndpoint = editingCoupon?.id
    ...
    const payload = {
      ...
      minPurchaseAmount:
        data.minPurchaseAmount === undefined ? null : data.minPurchaseAmount,
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: `Coupon ${editingCoupon ? 'Updated' : 'Added'}`,
          description: `Coupon "${result.code}" has been successfully ${
            editingCoupon ? 'updated' : 'added'
          }.`,
        });
        ...
        toast({
          title: `Error ${editingCoupon ? 'Updating' : 'Adding'} Coupon`,
      ...
      toast({
        title: 'Network Error',
        description: `Failed to ${editingCoupon ? 'update' : 'add'} coupon.`,
        variant: 'destructive',
      });
    } finally {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          ...
          description: `Coupon "${couponToDelete.code}" deleted.`,
        });
        ...
        toast({
          ...
          description: errorData.message || 'Could not delete coupon.',
          variant: 'destructive',
        });
      }
    } catch (error) {
    ...
    return coupons
      .filter((coupon) => {
        ...
        if (filterStatus === 'Active')
          statusMatch = coupon.isActive && expiry > now;
        else if (filterStatus === 'Inactive')
          statusMatch =
            !coupon.isActive &&
            expiry > now; // Still not expired but manually set to inactive
  ...
  if (isLoading && coupons.length === 0 && !initialCoupons.length) {
    // Show loading only if truly no data yet
    couponTableContent = (
                ...
                <TableRow key={coupon.id?.toString()}>
                  ...
                  <TableCell>
                    <ClientSideFormattedDate
                      isoDateString={coupon.expiryDate as string}
                      formatString="PP"
                    />
                  </TableCell>
    ...
    <>
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          >
            {isLoading ? (
              <LoadingSpinner size={16} className="mr-0 sm:mr-2" />
            ) : (
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!isSubmitting) setIsFormModalOpen(open);
        }}
      >
