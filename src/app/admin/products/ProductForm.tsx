...
import {
  ...
  Tag,
  ListPlus,
} from 'lucide-react'; // Added ListPlus
...
const colorSchema = z.object({
  ...
  image: z
    .string()
    .url({ message: 'Please enter a valid URL for color image.' })
    .optional()
    .or(z.literal('')),
  ...
  .object({
    ...
    discountType: z
      .enum(['percentage', 'fixed'], {
        invalid_type_error: 'Select a valid discount type or None.',
      })
      .optional(),
        ...
        z.object({
          url: z
            ...
            .refine(isValidImageUrl, {
              message:
                'Must be a valid URL (http/https) or an API path (/api/...).',
            }),
        })
      )
      .min(1, 'At least one image is required (upload or provide a path/URL).')
      .refine(
        (items) => items.every((item) => item.url && item.url.trim() !== ''),
        {
          message:
            'All image slots must have a valid path/URL. Remove empty slots or provide paths/URLs.',
          path: ['imageUrls'],
        }
      ),
  ...
  .superRefine((data, ctx) => {
    if (
      ...
      ctx.addIssue({
        ...
        message:
          'Discount value is required and must be positive when a discount type is selected.',
        path: ['discountValue'],
      });
    }
    ...
    if (data.discountType === 'percentage') {
      if (
        data.discountValue &&
        (data.discountValue < 1 || data.discountValue > 99)
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
...
export default function ProductForm({
  ...
  const form = useForm<ProductFormData>({
    ...
    defaultValues: initialData
      ? {
          ...
          imageUrls: initialData.imageUrls?.map((url) => ({
            url: url || '',
          })) || [{ url: '' }],
          colors:
            initialData.colors?.length > 0
              ? initialData.colors.map((c) => ({ ...c, image: c.image || '' }))
              : defaultValues.colors,
          sizes:
            initialData.sizes?.length > 0
              ? initialData.sizes.map((s) => ({ value: s }))
              : [{ value: '' }],
          specifications:
  ...
  useEffect(() => {
    ...
    if (initialData?.imageUrls) {
      setImagePreviewKeys(
        initialData.imageUrls.map((_, i) => `${i}-${timestamp}`)
      );
    } else {
      setImagePreviewKeys(imageUrlFields.map((_, i) => `${i}-${timestamp}`));
    }
  ...
  const handleProductImageUpload = async (
    ...
    try {
      ...
      if (response.ok && result.filename) {
        ...
        form.setValue(`imageUrls.${index}.url`, retrievalPath, {
          shouldValidate: true,
        });
        const timestamp = Date.now();
        setImagePreviewKeys((prev) => {
          const newKeys = [...prev];
          newKeys[index] = `${index}-${timestamp}`;
          return newKeys;
        });
        toast({
          title: 'Product Image Uploaded to GridFS',
          description: `Image path set to: ${retrievalPath}. Save product to apply.`,
        });
      } else {
        toast({
          title: 'Upload Failed',
          description: result.message || 'Could not process image via GridFS.',
          variant: 'destructive',
        });
      }
    } catch (error) {
      toast({
        title: 'Upload Error',
        description: 'An error occurred during image processing with GridFS.',
        variant: 'destructive',
      });
    } finally {
  ...
  const onSubmit: SubmitHandler<ProductFormData> = async (data) => {
    ...
    if (data.imageUrls.some((img) => !isValidImageUrl(img.url))) {
      toast({
        ...
        description:
          'All image slots must have a valid URL (e.g., http://...) or an API path (e.g., /api/...). Remove empty slots or provide valid paths/URLs.',
        variant: 'destructive',
      });
      return;
    }
    ...
    const discountTypeForPayload =
      discountValueForPayload === null ? null : data.discountType;
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: `"${resultProduct.name}" has been successfully ${
      ...
      toast({
        ...
        description: `Failed to ${
          mode === 'add' ? 'add' : 'update'
        } product. Please check your connection.`,
        variant: 'destructive',
      });
    }
  };
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Card className="p-4 pt-2">
          ...
          <FormDescription className="mb-3">
            Set a percentage or fixed amount discount.
          </FormDescription>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
            <FormField
              control={form.control}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input
                  placeholder="e.g., 'elegant dress' or 'mens casual shirt'"
                  {...field}
                  disabled={isUploadingImage || form.formState.isSubmitting}
                />
              </FormControl>
              <FormDescription>
                Optional. Keywords for AI image generation if product images are
                missing or for related content.
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
        ...
        <Card className="p-4 pt-2">
          ...
          <FormDescription className="mb-2">
            Add at least one image. Enter a valid path/URL or upload a file. The
          ...
          {imageUrlFields.map((field, index) => (
            <div
              key={field.id}
              className="space-y-3 mb-4 pb-4 border-b last:border-b-0 last:pb-0"
            >
              <FormField
                control={form.control}
                ...
                render={({ field: itemField }) => (
                  <FormItem>
                    <FormLabel
                      htmlFor={`imageUrls.${index}.url`}
                      className="text-xs"
                    >
                      Image Path/URL {index + 1}
                    </FormLabel>
              ...
              />
              {form.watch(`imageUrls.${index}.url`) &&
                isValidImageUrl(form.watch(`imageUrls.${index}.url`)) && (
                  <div className="mt-2 p-2 border rounded-md inline-block bg-muted/30">
                    <Image
                      key={imagePreviewKeys[index] || `${index}-default`}
                      ...
                      onError={(e) => {
                        console.error(
                          'Error loading product image preview:',
                          form.watch(`imageUrls.${index}.url`),
                          e
                        );
                      }}
          ...
          <Button
            ...
            onClick={() => {
              ...
              setImagePreviewKeys((prev) => [
                ...prev,
                `${imageUrlFields.length}-${timestamp}`,
              ]);
            }}
            className="mt-2"
            disabled={isUploadingImage || form.formState.isSubmitting}
        ...
        <Card className="p-4 pt-2">
          ...
          <FormDescription className="mb-2">
            Define color options. The main product image can be used as fallback
            if a color-specific image is not provided.
          </FormDescription>
          {colorFields.map((field, index) => (
            <Card
              key={field.id}
              className="mb-4 p-3 space-y-3 relative bg-muted/20"
            >
              ...
              <FormField
                ...
                render={({ field: itemField }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        placeholder="https://example.com/color-image.png"
                        {...itemField}
                        disabled={
                          isUploadingImage || form.formState.isSubmitting
                        }
                    ...
                    <FormDescription>
                      If blank, first main product image will be used.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field: itemField }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        placeholder="e.g., 'blue t-shirt model'"
                        {...itemField}
                        disabled={
                          isUploadingImage || form.formState.isSubmitting
                        }
          ...
          <Button
            ...
            size="sm"
            onClick={() =>
              appendColor({ name: '', hex: '#000000', image: '', aiHint: '' })
            }
            className="mt-2"
            disabled={isUploadingImage || form.formState.isSubmitting}
          >
        ...
        <Card className="p-4 pt-2">
          ...
          <FormDescription className="mb-2">
            Add product sizes (e.g., S, M, L, XL, 32, 40). Clear an input to
            remove a size.
          </FormDescription>
          {sizeFields.map((field, index) => (
            <div
              key={field.id}
              className="flex items-center gap-2 mb-3 p-3 border rounded-md bg-muted/20"
            >
              <FormField
                control={form.control}
        ...
        <Card className="p-4 pt-2">
          ...
          <FormDescription className="mb-3">
            Add any other relevant product details (e.g., Material, Origin,
            Warranty).
          </FormDescription>
          {specificationFields.map((field, index) => (
            <div
              key={field.id}
              className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_auto] gap-3 mb-3 p-3 border rounded-md bg-muted/20 items-end"
            >
              <FormField
                control={form.control}
