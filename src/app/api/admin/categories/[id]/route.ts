...
export async function DELETE(
  ...
  try {
    ...
    console.log(
      `[API /api/admin/categories/[id]] Category ${id} deleted successfully.`
    );
    return NextResponse.json(
    ...
    console.error(
      `[API /api/admin/categories/[id]] DELETE Error for ID ${id}:`,
      error
    );
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    const body: { name: string; imageUrl?: string; aiHint?: string } =
      await req.json();
    console.log(
      `[API /api/admin/categories/[id]] Update body for ID ${id}:`,
      body
    );
    ...
    if (!updatedCategory) {
      return NextResponse.json(
        {
          message: 'Category not found or update failed for an unknown reason.',
        },
        { status: 404 }
      );
    }
    ...
    console.log(
      `[API /api/admin/categories/[id]] Category ${id} updated successfully.`
    );
    return NextResponse.json(updatedCategory, { status: 200 });
  } catch (error: any) {
    console.error(
      `[API /api/admin/categories/[id]] PUT Error for ID ${id}:`,
      error
    );
