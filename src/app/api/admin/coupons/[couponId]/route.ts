...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { message: 'Failed to fetch coupon', error: error.message },
      { status: 500 }
    );
  }
}
...
export async function PUT(
  ...
  try {
    const body: Partial<
      Omit<Coupon, 'id' | '_id' | 'createdAt' | 'updatedAt' | 'usageCount'>
    > = await req.json();
    ...
    if (!updatedCoupon) {
      return NextResponse.json(
        { message: 'Coupon not found or update failed.' },
        { status: 404 }
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (!success) {
      return NextResponse.json(
        { message: 'Coupon not found or delete failed.' },
        { status: 404 }
      );
    }
    return NextResponse.json(
