...
export async function PUT(
  ...
  if (!orderId) {
    return NextResponse.json(
      { message: 'Order ID is required in the path.' },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    console.log(
      `[API /api/admin/orders/${orderId}/status] Attempting to update status to: ${newStatus}`
    );
    ...
    if (updatedOrder) {
      console.log(
        `[API /api/admin/orders/${orderId}/status] Order status updated successfully.`
      );
      return NextResponse.json(updatedOrder, { status: 200 });
    } else {
      console.warn(
        `[API /api/admin/orders/${orderId}/status] Order not found or update failed.`
      );
      return NextResponse.json(
        { message: 'Order not found or failed to update status.' },
        { status: 404 }
      );
    }
  } catch (error: any) {
    console.error(
      `[API /api/admin/orders/${orderId}/status] Error updating order status:`,
      error
    );
    if (
