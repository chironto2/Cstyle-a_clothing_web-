...
export async function GET(
  ...
  try {
    ...
    console.error(
      `[API /api/admin/products/[id]] GET Error for ID ${id}:`,
      error
    );
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    const body: Partial<Omit<Product, '_id' | 'createdAt' | 'updatedAt'>> & {
      _id?: string;
    } = await req.json();
    console.log(
      `[API /api/admin/products/[id]] Update body for ID ${id}:`,
      body
    );
    ...
    if (
      discountType === 'fixed' &&
      discountValue &&
      Number(discountValue) >= Number(price)
    ) {
      return NextResponse.json(
        {
          message: 'Fixed discount value must be less than the regular price.',
        },
        { status: 400 }
      );
    }
    if (
      ...
      discountValue &&
      (Number(discountValue) < 1 || Number(discountValue) > 99)
    ) {
      return NextResponse.json(
    ...
    if (discountValue && Number(discountValue) > 0 && !discountType) {
      return NextResponse.json(
        {
          message:
            'Discount type is required if discount value is provided and positive.',
        },
        { status: 400 }
      );
    }
    ...
    if (result.modifiedCount === 0 && result.matchedCount > 0) {
      console.log(
        `[API /api/admin/products/[id]] Product ${id} data was the same, no modification needed.`
      );
    }
    ...
    if (!updatedProduct)
      return NextResponse.json(
        { message: 'Product not found after successful update operation' },
        { status: 404 }
      );
    ...
    const productWithStringId = {
      ...
      discountValue:
        updatedProduct.discountValue === null ||
    ...
    console.log(
      `[API /api/admin/products/[id]] Product ${id} updated successfully.`
    );
    return NextResponse.json(productWithStringId, { status: 200 });
  } catch (error: any) {
    console.error(
      `[API /api/admin/products/[id]] PUT Error for ID ${id}:`,
      error
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.log(
      `[API /api/admin/products/[id]] Product ${id} deleted successfully.`
    );
    return NextResponse.json(
    ...
    console.error(
      `[API /api/admin/products/[id]] DELETE Error for ID ${id}:`,
      error
    );
    return NextResponse.json(
