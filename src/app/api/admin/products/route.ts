...
export async function POST(req: NextRequest) {
  try {
    const body: Omit<Product, '_id' | 'id' | 'createdAt' | 'updatedAt'> =
      await req.json();
    ...
    if (
      ...
      discountValue &&
      (discountValue < 1 || discountValue > 99)
    ) {
      return NextResponse.json(
    ...
    if (discountValue && discountValue > 0 && !discountType) {
      return NextResponse.json(
        { message: 'Discount type is required if discount value is provided.' },
        { status: 400 }
      );
    }
    ...
    const newProduct: Omit<Product, '_id' | 'id'> = {
      ...
      discountValue:
        discountValue && discountValue > 0 && discountType
