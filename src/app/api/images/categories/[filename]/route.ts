...
export async function GET(
  ...
  console.log(
    `[API /api/images/categories/[filename]] GET request for filename: ${filename}`
  );
  ...
  try {
    ...
    if (!files || files.length === 0) {
      console.warn(
        `[API /api/images/categories/[filename]] File not found in GridFS: ${filename}`
      );
    ...
    const readableStream = new ReadableStream({
      start(controller) {
        ...
        downloadStream.on('error', (err) => {
          console.error(
            `[API /api/images/categories/[filename]] Stream error for ${filename}:`,
            err
          );
    ...
    console.log(
      `[API /api/images/categories/[filename]] Streaming file: ${filename} with contentType: ${fileDoc.contentType}`
    );
    return new NextResponse(readableStream, { status: 200, headers });
  } catch (error: any) {
    console.error(
      `[API /api/images/categories/[filename]] Error serving file ${filename}:`,
      error
    );
    return NextResponse.json(
      { message: 'Failed to serve image', error: error.message },
      { status: 500 }
    );
  }
}
