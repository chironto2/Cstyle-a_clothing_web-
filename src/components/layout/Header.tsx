...
export default function Header() {
  ...
  const updateCartCount = useCallback(async () => {
    if (!isAuthenticated) {
      console.log(
        '[Header] updateCartCount - Not authenticated, setting count to 0'
      );
      setCartItemCount(0);
      return;
    }
    ...
    console.log(
      '[Header] updateCartCount - Authenticated, fetching DB cart count'
    );
    try {
  ...
  const updateWishlistCount = useCallback(async () => {
    if (!isAuthenticated) {
      console.log(
        '[Header] updateWishlistCount - Not authenticated, setting count to 0'
      );
    ...
    if (!token) {
      console.log(
        '[Header] updateWishlistCount - No token, setting count to 0'
      );
      setWishlistItemCount(0);
      return;
    }
    console.log(
      '[Header] updateWishlistCount - Authenticated, fetching DB wishlist count'
    );
    try {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (event: StorageEvent) => {
      if (
        ...
        console.log(
          '[Header] Storage event triggered for auth keys or clear all. Re-checking auth.'
        );
        checkAuth();
      }
    };
    ...
    const handleCartUpdate = () => {
      console.log(
        '[Header] cartUpdated event received, re-fetching cart count if authenticated.'
      );
    ...
    const handleWishlistUpdate = () => {
      console.log(
        '[Header] wishlistUpdated event received, re-fetching wishlist count if authenticated.'
      );
      if (isAuthenticated) updateWishlistCount();
      else setWishlistItemCount(0);
    };
  ...
  useEffect(() => {
    console.log(
      '[Header] isAuthenticated changed to:',
      isAuthenticated,
      'Updating counts accordingly.'
    );
    if (isAuthenticated) {
      updateCartCount();
      updateWishlistCount();
  ...
  const wishlistLink = isAuthenticated
    ? '/dashboard/wishlist'
    : '/login?redirect=/dashboard/wishlist';
        ...
        <div className="bg-card border-b border-border shadow-sm">
          <Container className="flex items-center justify-between h-16 px-4">
            <div className="flex items-center">
              <Link
                href="/"
                className="text-3xl font-headline font-bold text-primary hover:text-primary/80 transition-colors mr-6"
                aria-label="CstyleN Home"
              >
                Cstyle
              </Link>
                ...
                <SheetContent side="right" className="w-[280px] bg-card p-6">
                  <SheetHeader>
                    ...
                    <Link
                      ...
                      className="text-2xl font-headline font-bold text-primary mb-6 block"
                    >
                      Cstyle
                    </Link>
                  </SheetHeader>
