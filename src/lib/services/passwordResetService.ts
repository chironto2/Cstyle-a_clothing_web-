 ...
 */
async function generateAndStoreResetToken(
  userId: string
): Promise<{ token: string; expiresAt: Date } | null> {
  console.log(
    `[passwordResetService] generateAndStoreResetToken called for userId: ${userId}`
  );
  if (!ObjectId.isValid(userId)) {
    console.error(
      '[passwordResetService] Invalid user ID format for creating reset token.'
    );
  ...
  try {
    ...
    const tokensCollection = db.collection<PasswordResetTokenDoc>(
    ...
    const result = await tokensCollection.insertOne(
      newTokenDocument as PasswordResetTokenDoc
    );
    ...
    console.log(
      `[passwordResetService] Password reset token generated and stored for userId: ${userId}`
    );
    return { token, expiresAt };
  } catch (error: any) {
    console.error(
      `[passwordResetService] Error generating/storing reset token for userId ${userId}:`,
      error.message
    );
...
export async function requestPasswordReset(email: string): Promise<boolean> {
  console.log(
    `[passwordResetService] requestPasswordReset called for email: ${email}`
  );
  try {
    const { db } = await connectToDatabase();
    ...
    if (!user) {
      console.log(
        `[passwordResetService] User with email ${email} not found. No email will be sent.`
      );
    ...
    }/reset-password?token=${tokenData.token}`;

    const emailSubject = 'Password Reset Request - Cstyle Fashion';
    const emailHtmlBody = `
      <p>Hello ${user.name || 'User'},</p>
      <p>You requested a password reset for your Cstyle Fashion account.</p>
      ...
      <p>Thank you,<br/>The Cstyle Fashion Team</p>
    `;
    const emailTextBody = `
      Hello ${user.name || 'User'},
      You requested a password reset for your Cstyle Fashion account.
      Please visit the following link to set a new password. This link will expire in 1 hour:
      ${resetUrl}
      If you did not request a password reset, please ignore this email or contact support if you have concerns.
      Thank you,
      The Cstyle Fashion Team
    `;
    ...
    console.log(
      `[passwordResetService] Password reset email successfully dispatched to: ${user.email}`
    );
    return true; // Email sending process initiated
  } catch (error: any) {
    console.error(
      `[passwordResetService] Error in requestPasswordReset for email ${email}:`,
      error.message
    );
...
export async function verifyPasswordResetToken(
  ...
  console.log(
    `[passwordResetService] verifyPasswordResetToken called for token: ${
      token ? token.substring(0, 10) + '...' : 'null'
    }`
  );
  if (!token || typeof token !== 'string') {
    console.warn(
      '[passwordResetService] Invalid or missing token for verification.'
    );
    return null;
  }
  ...
  try {
    ...
    if (!tokenDoc) {
      console.warn(
        `[passwordResetService] Token not found: ${token.substring(0, 10)}...`
      );
      return null;
    }
    ...
    if (tokenDoc.expiresAt < new Date()) {
      console.warn(
        `[passwordResetService] Token expired for ${token.substring(0, 10)}...`
      );
    ...
    console.log(
      `[passwordResetService] Token verified successfully for userId: ${tokenDoc.userId}`
    );
    return tokenDoc;
  } catch (error: any) {
    console.error(
      `[passwordResetService] Error verifying password reset token:`,
      error.message
    );
    throw error;
  }
...
export async function deletePasswordResetToken(
  ...
  console.log(
    `[passwordResetService] deletePasswordResetToken called for tokenId: ${tokenId.toString()}`
  );
  const objectIdToDelete =
    typeof tokenId === 'string' ? new ObjectId(tokenId) : tokenId;
  if (!ObjectId.isValid(objectIdToDelete)) {
    console.error(
      '[passwordResetService] Invalid token ID format for deletion.'
    );
    return false;
  }
  ...
  try {
    ...
    console.error(
      `[passwordResetService] Error deleting password reset token ${tokenId.toString()}:`,
      error.message
    );
    throw error;
  }
